...
SELECT COUNT(p.*), p.id, p.created_at, p.updated_at, p.title, p.url, p.description, p.published_at, p.feed_id
FROM posts AS p
INNER JOIN
...
func (q *Queries) GetPostsFromUser(ctx context.Context, userID uuid.UUID) ([]GetPostsFromUserRow, error) {
